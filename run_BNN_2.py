...
class BayesLinear(nn.Module):
    ...
    def forward(self, input):
        ...
        """
        # sample weights
        w_epsilon = Normal(0,0.1).sample(self.w_mu.shape).cuda()
        self.w = self.w_mu + torch.log(1+torch.exp(self.w_rho)) * w_epsilon

        # sample bias
        b_epsilon = Normal(0,0.1).sample(self.b_mu.shape).cuda()
...
class BNN(nn.Module):
    def __init__(self, input_features, hidden_units, output_features, noise_tol=.1,  prior_var=1.):
        ...
        self.decoder = nn.ModuleList([
            BayesLinear(hidden_units, 512, prior_var=prior_var),
            BayesLinear(512, output_features, prior_var=prior_var)
        ])
    ...
    def forward(self, x):
        ...
        y = F.relu(self.decoder[0](y))
        y = self.decoder[1](y)
        return y

    def log_prior(self):
        # calculate the log prior over all the layers
        return self.decoder[0].log_prior + self.decoder[1].log_prior

    def log_post(self):
        # calculate the log posterior over all the layers
        return self.decoder[0].log_post + self.decoder[1].log_post
